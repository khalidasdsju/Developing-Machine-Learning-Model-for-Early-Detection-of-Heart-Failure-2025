...
def create_optimized_models():
    ...
    xgboost = XGBClassifier(
        ...
        n_estimators=500,  # Reduced from 955 for efficiency
        min_child_weight=1,
        subsample=0.9744,
    ...
    random_forest = RandomForestClassifier(
        n_estimators=200,  # Reduced from 298 for efficiency
        max_depth=20,      # Slightly reduced from 23
        min_samples_split=6,
        min_samples_leaf=4,
        max_features='log2',
        bootstrap=False,
...
def cross_validate_model(model, X, y, n_folds=5):  # Reduced from 10 to 5 folds
...
def main():
    ...
    for model_name, model in all_models.items():
        ...
        # Perform cross-validation
        cv_result = cross_validate_model(model, X, y, n_folds=5)  # Reduced from 10 to 5
    ...
    # Save only the best model to save time
    print(f"Saving only the best model to save time")
    ...
    print("-"*80)
    print(f"5-Fold CV Accuracy: {results[best_model_name]['cv']['accuracy'][0]:.4f} ± {results[best_model_name]['cv']['accuracy'][1]:.4f}")
    print("-"*80)
